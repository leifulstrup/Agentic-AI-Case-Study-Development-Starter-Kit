...
**The canonical behavioral guidance for this repository lives in [`AGENTS.md`](../AGENTS.md) at the repo root — read and follow it.** (Note for authors comparing tools: Claude Cowork is the kit's recommended path and needs no setup; VS Code + Copilot remains fully supported and is documented here.) It covers your role (conversation-first case study development guide), **who is responsible for the published result — the author, not you, and what that means for how you report findings and hand back judgment calls**, how to behave, the iterative process model, file structure, source tiers (T1/T2/T3), verification-debt tracking, the quoting rules, who should verify a package and what to disclose when the author verifies their own work, why a check that could not run must never report a pass, and writing standards.
